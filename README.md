# Projeto Final – Visão Computacional  
## Inspeção Visual Automática para Classificação de Frutas Frescas vs Podres

---

**Alunos:** Daniel Augusto de Campos da Silva, Gustavo Razera e Leonardo Souza Furiski  
**Cenário:** A – Inspeção de frutas em uma central de distribuição  
**Dataset:** Fruits fresh and rotten for classification: https://www.kaggle.com/datasets/sriramr/fruits-fresh-and-rotten-for-classification  
**Classes:** `fresh` (1143 imagens) e `rotten` (1143 imagens) com `freshapples` (381 imagens), `freshbanana` (381 imagens), `freshoranges` (381 imagens), `rottenapples` (381 imagens), `rottenbanana` (381 imagens) e `rottenoranges` (381 imagens)  
**Classificacao:** Random Forest e SVM.

---

## Bibliotecas

- opencv-python
- numpy
- pandas
- matplotlib
- seaborn
- scikit-image
- scikit-learn

---

## Instalação e dependências

```bash
pip list
python.exe -m ensurepip
pip install opencv-python numpy pandas matplotlib seaborn scikit-image scikit-learn
